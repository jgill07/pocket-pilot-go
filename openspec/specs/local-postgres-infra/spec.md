...
Provide the local development infrastructure for pocket-pilot's Postgres backend:
a Docker Compose Postgres service, command-driven golang-migrate schema
migrations, Make targets that orchestrate the workflow, and `DATABASE_URL`-based
connection configuration. This is infrastructure only — it stands up the database
and its schema for local dev without embedding migrations in the app binary.
